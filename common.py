...
def hash_game(server_salt, client_salt, ships):
    hashed = hashlib.pbkdf2_hmac('sha1', bytes( ships[0] ), client_salt.encode() , 1)

    for s in ships[1:]:
        hashed = hashlib.pbkdf2_hmac('sha1', hashed, bytes( s ), 1)

    hashed = hashlib.pbkdf2_hmac('sha1', hashed, server_salt.encode() , 1)
